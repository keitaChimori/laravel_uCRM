...
<template>
              ...
              <tbody>
                <tr v-for="customer in customers.value.data" :key="customer.id">
                  ...
                  <td class="px-4 py-3 border-b-2 border-gray-200">
                    <button @click="setCustomer({id:customer.id, kana:customer.kana})" type="button" class="text-blue-600">
                      {{ customer.name }}
                    </button>
                  </td>
  ...
  <!-- 会員名入力フォーム -->
  <input name="customer" v-model="search" class="w-full bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
